...
import com.lingyan.banquet.databinding.FragmentCelStep6Binding;
...
import com.lingyan.banquet.ui.celebration.session.EndSessionFragment;
...
public class CelStep6Fragment extends BaseCelStepFragment {
    private NetCelRestoreStep6.DataDTO mData;

    public static CelStep6Fragment newInstance() {
        CelStep6Fragment fragment = new CelStep6Fragment();
        return fragment;
    }

    private FragmentManager mFragmentManager;
    private List<EndSessionFragment> mFragmentList;
    private FragmentCelStep6Binding mBinding;


    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        mBinding = FragmentCelStep6Binding.inflate(inflater);
...
//                mData.setStatus(null);
    ...
    public void setMoney() {
        List<NetCelRestoreStep6.DataDTO.BanquetNumDTO> list = mData.getBanquetNum();
        if (ObjectUtils.isEmpty(list)) {
            return;
        }
        BigDecimal all = new BigDecimal(0);
        for (int i = 0; i < list.size(); i++) {
            NetCelRestoreStep6.DataDTO.BanquetNumDTO dto = list.get(i);
    ...
    private void restoreDataFromNet() {
        OkGo.<NetCelRestoreStep6>post(HttpURLs.banquetGetInfo)
                .params("id", getCelId())
                .params("step", 6)
                .tag(getThisFragment())
                .execute(new JsonCallback<NetCelRestoreStep6>() {
                    @Override
                    public void onSuccess(Response<NetCelRestoreStep6> response) {
                        NetCelRestoreStep6 body = response.body();
                        mData = body.getData();
