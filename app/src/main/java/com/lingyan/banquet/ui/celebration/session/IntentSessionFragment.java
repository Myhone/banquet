...
import com.lingyan.banquet.databinding.FragmentCelIntentSessionBinding;
...
public class IntentSessionFragment extends BaseFragment {
    ...
    private FragmentCelIntentSessionBinding mBinding;

    private NetCelRestoreStep2.DataDTO.BanquetNumDTO mDTO;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        mBinding = FragmentCelIntentSessionBinding.inflate(inflater);
        ...
        mBinding.tvMealName.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                //庆典套餐
                OkGo.<NetMealList>post(HttpURLs.listCelebration)
//                        .cacheMode(CacheMode.IF_NONE_CACHE_REQUEST)
    ...
    private void showSelectHallDialog() {
        ...
        List<NetCelRestoreStep2.DataDTO.BanquetNumDTO.HallInfoDTO> hallInfo = mDTO.getHall_info();
        if (ObjectUtils.isNotEmpty(hallInfo)) {
            for (NetCelRestoreStep2.DataDTO.BanquetNumDTO.HallInfoDTO dto : hallInfo) {
        ...
        dialog.setOnMultipleSelectListener(new TwoLineTabSelectDialog.OnMultipleSelectListener() {
            ...
            public void OnMultipleSelect(List<String> idList, List<String> nameList, TwoLineTabSelectDialog dialog) {
                ...
                dialog.dismiss();
                List<NetCelRestoreStep2.DataDTO.BanquetNumDTO.HallInfoDTO> hallInfoList = new ArrayList<>();

                for (int i = 0; i < idList.size(); i++) {
                    String id = idList.get(i);
                    String name = nameList.get(i);
                    NetCelRestoreStep2.DataDTO.BanquetNumDTO.HallInfoDTO hallInfoDTO = new NetCelRestoreStep2.DataDTO.BanquetNumDTO.HallInfoDTO();
        ...
        new PickerListDialog(getActivity())
                ...
                .itemSelectedCallBack(new PickerListDialog.ItemSelectedCallBack() {
                    ...
                    public void onItemSelected(int position, String text, PickerListDialog dialog) {
                        ...
                        mDTO.setCelebration_id(dataDTO.getId());
                        mDTO.setCelebration_name(dataDTO.getName());
                    }
                })
                .show();
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        changeUI();
        //计划桌数
        mBinding.etColorSystem.addTextChangedListener(new TextWatcherImpl() {
            @Override
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                //色系
                String trim = s.toString().trim();
                mDTO.setColor_system(trim);
            }

        });
        mBinding.etStyle.addTextChangedListener(new TextWatcherImpl() {
            @Override
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                //庆典风格
                String trim = s.toString().trim();
    ...
    public void setData(NetCelRestoreStep2.DataDTO.BanquetNumDTO dto) {
        mDTO = dto;
        if (mDTO == null) {
            mDTO = new NetCelRestoreStep2.DataDTO.BanquetNumDTO();
        }
    ...
    private void changeUI() {
        if (mDTO != null) {
            List<NetCelRestoreStep2.DataDTO.BanquetNumDTO.HallInfoDTO> hallInfoDTOList = mDTO.getHall_info();

            mBinding.llRoomContainer.removeAllViews();
            if (ObjectUtils.isNotEmpty(hallInfoDTOList)) {
                for (int i = 0; i < hallInfoDTOList.size(); i++) {
                    NetCelRestoreStep2.DataDTO.BanquetNumDTO.HallInfoDTO hallInfoDTO = hallInfoDTOList.get(i);
                    if (i == 0) {
            ...
            mBinding.etStyle.setText(mDTO.getStyle());
            mBinding.etColorSystem.setText(mDTO.getColor_system());
            mBinding.tvMealName.setText(mDTO.getCelebration_name());
